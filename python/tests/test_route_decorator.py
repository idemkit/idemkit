...
from idemkit import Idempotency, InMemoryBackend
...
async def test_protected_route_deduplicates() -> None:
    ...
    idem = Idempotency(
        ...
        scope=lambda req: req.headers.get("x-user-id", "anon"),
    )
...
async def test_only_decorated_route_is_protected() -> None:
    ...
    protected = {"n": 0}
    plain = {"n": 0}
    idem = Idempotency(backend=InMemoryBackend(), scope=lambda req: "u")
    ...
    assert protected["n"] == 1  # deduplicated
    assert plain["n"] == 2  # not protected
...
async def test_decorator_sees_real_request_for_identity() -> None:
    ...
    idem = Idempotency(
        backend=InMemoryBackend(),
        scope=lambda req: req.headers["X-User-Id"],  # capitalized
    )
    ...
    assert calls["n"] == 2  # different users isolated despite same key
    assert ra.json() == {"u": "a"}
    assert rb.json() == {"u": "b"}
...
async def test_decorator_payload_mismatch_raises_typed_exception() -> None:
    ...
    app = Starlette(routes=[Route("/p", endpoint, methods=["POST"])])
    # raise_app_exceptions so the typed exception surfaces in the test client.
