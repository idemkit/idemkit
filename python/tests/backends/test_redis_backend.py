...
async def _make_client() -> Any:
    if REDIS_URL:
        ...
        await client.flushdb()
        return client
    return fakeredis_aio.FakeRedis(decode_responses=False)


async def test_namespace_isolates_keys_on_one_redis() -> None:
    client = await _make_client()
    a = RedisBackend(client, namespace="app-a")
    b = RedisBackend(client, namespace="app-b")
    try:
        # Same effective_key, different namespaces: both are NEW (fully isolated).
        ra = await a.claim("shared", "fp", 1, 30.0)
        rb = await b.claim("shared", "fp", 1, 30.0)
        assert ra.result is ClaimResultType.NEW_CLAIMED
        assert rb.result is ClaimResultType.NEW_CLAIMED
        assert ra.our_claim_token != rb.our_claim_token
        # The raw Redis keys are namespace-prefixed; the bare key is untouched.
        assert await client.get("app-a:shared") is not None
        assert await client.get("app-b:shared") is not None
        assert await client.get("shared") is None
    finally:
        # a and b share one client; close it once (no subscriber tasks were started).
        await client.aclose()


async def test_namespace_in_flight_wait_still_notifies() -> None:
    client = await _make_client()
    backend = RedisBackend(client, namespace="ns1")
    try:
        first = await backend.claim("wait-key", "fp", 1, 30.0)

        async def waiter() -> Any:
            return await backend.wait_for_completion("wait-key", timeout_seconds=2.0)

        async def completer() -> None:
            await asyncio.sleep(0.05)
            await backend.complete("wait-key", first.our_claim_token, 200, {}, b"ok", 60.0)

        record, _ = await asyncio.gather(waiter(), completer())
        assert record is not None
        assert record.state is State.COMPLETED
