...
async def test_custom_table_isolates_records() -> None:
    assert PG_URL is not None
    table = "idem_custom_" + uuid.uuid4().hex[:8]
    await init_pg(PG_URL, table=table)
    backend = PostgresBackend.from_url(PG_URL, table=table, min_size=1, max_size=4)
    try:
        key = _k("custom")
        r1 = await backend.claim(key, "fp", 1, 30.0)
        assert r1.result is ClaimResultType.NEW_CLAIMED
        assert await backend.complete(key, r1.our_claim_token, 200, {}, b"body", 60.0)

        # The row landed in the custom table, not the default one.
        conn = await asyncpg.connect(PG_URL)
        try:
            n_custom = await conn.fetchval(
                f"SELECT count(*) FROM {table} WHERE effective_key = $1", key
            )
            n_default = await conn.fetchval(
                "SELECT count(*) FROM idemkit_records WHERE effective_key = $1", key
            )
        finally:
            await conn.close()
        assert n_custom == 1
...
def test_invalid_table_name_is_rejected() -> None:
    ...
    for bad in ["bad; DROP TABLE x", "has space", "1leading_digit", "a" * 47]:
        with pytest.raises(ConfigurationError):
            PostgresBackend(table=bad)
