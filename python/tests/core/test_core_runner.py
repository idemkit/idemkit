...
from idemkit.core.state import Decision  # noqa: E402
...
def _core(backend, *, events=None, **overrides) -> IdempotentCore:
    return IdempotentCore(
        backend,
        emitter=EventEmitter([events.append] if events is not None else None),
...
async def test_concurrent_run_once_executes_exactly_once(backend) -> None:
    ...
    results = await asyncio.gather(*(core.run_once(key, "fp", handler) for _ in range(parallel)))
    ...
    assert (
        statuses[RunStatus.EXECUTED] + statuses[RunStatus.REPLAYED] + statuses[RunStatus.CONFLICT]
        == parallel
    )
    for r in results:
...
async def test_heartbeat_lease_lost_cancels_handler(backend) -> None:
    ...
    and the operation reports LEASE_LOST — it did NOT finish (spec §5.3.1). A
    lease_lost event is emitted so the partition hazard is alertable."""
    events: list = []
    core = _core(backend, lease_ttl_seconds=0.3, events=events)
    key = _k("hb-lost")
    finished = False
    ...
    result = await core.run_once(key, "fp", handler, heartbeat=True, heartbeat_interval_seconds=0.1)
    ...
    assert any(e.decision is Decision.LEASE_LOST for e in events)


async def test_complete_failure_emits_event(backend) -> None:
    """If the backend fails to persist the result, the side effect already ran, so
    the claim is held (lease governs re-execution) and a complete_failed event is
    emitted so the at-least-once degradation is measurable."""
    events: list = []
    core = _core(backend, events=events)
    key = _k("complete-fail")

    async def boom(*args, **kwargs):
        raise StorageError("backend.complete blew up")

    backend.complete = boom  # type: ignore[method-assign]

    async def handler() -> StoredResult:
        return StoredResult(blob=b"ran")

    result = await core.run_once(key, "fp", handler)
    assert result.status is RunStatus.EXECUTED  # the handler ran
    assert any(e.decision is Decision.COMPLETE_FAILED for e in events)
...
async def test_response_hook_post_processes_replay(backend) -> None:
    ...
    def hook(stored: StoredResult) -> StoredResult:
        return StoredResult(blob=stored.blob + b"-hooked", meta=stored.meta, marker=stored.marker)
