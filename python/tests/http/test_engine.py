...
async def test_response_hook_modifies_only_the_replay() -> None:
    def hook(body: bytes, headers: dict[str, str], status: int) -> tuple[bytes, dict[str, str]]:
        headers["x-replayed-by-hook"] = "1"
        return body + b" (cached)", headers

    engine, _ = _make_engine(response_hook=hook)
    first = await engine.decide(_req())
    assert first.kind == "pass_through"  # the hook does NOT run on the first request
    await engine.on_complete(
        first.effective_key,
        first.claim_token,
        NeutralResponse(status=201, headers={"content-type": "application/json"}, body=b"ok"),
    )

    second = await engine.decide(_req())
    assert second.kind == "replay"
    assert second.response is not None
    assert second.response.headers["x-replayed-by-hook"] == "1"
    assert second.response.body == b"ok (cached)"
    assert second.response.headers["idempotency-replayed"] == "true"


async def test_response_hook_failure_falls_back_to_unmodified_replay() -> None:
    def bad_hook(body: bytes, headers: dict[str, str], status: int) -> tuple[bytes, dict[str, str]]:
        raise RuntimeError("boom")

    engine, _ = _make_engine(response_hook=bad_hook)
    first = await engine.decide(_req())
    await engine.on_complete(
        first.effective_key,
        first.claim_token,
        NeutralResponse(status=201, headers={}, body=b"ok"),
    )
    second = await engine.decide(_req())
    assert second.kind == "replay"
    assert second.response is not None
    assert second.response.body == b"ok"  # unmodified
    assert second.response.headers["idempotency-replayed"] == "true"
