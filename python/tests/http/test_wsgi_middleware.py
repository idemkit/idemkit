...
def test_duplicate_replays_handler_runs_once():
    ...
    mw = WSGIIdempotencyMiddleware(
        ...
        config=HttpConfig(scope=lambda req: "tenant-1"),
    )
...
def test_missing_key_required_returns_400():
    ...
    mw = WSGIIdempotencyMiddleware(
        _counting_app(counter),
        backend=InMemoryBackend(),
        config=HttpConfig(scope=lambda req: "tenant-1", require_key_for_mutations=True),
    )
...
def test_oversized_request_bypasses_idempotency():
    ...
    mw = WSGIIdempotencyMiddleware(
        _counting_app(counter),
        backend=InMemoryBackend(),
        config=HttpConfig(scope=lambda req: "tenant-1", max_request_body_bytes=16),
    )
    big = b'{"payload": "' + b"x" * 100 + b'"}'
