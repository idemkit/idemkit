...
async def test_set_cookie_header_not_replayed() -> None:
    ...
    assert "set-cookie" in {k.lower() for k in r1.headers}
    set_cookie_replay = [h for h in r2.headers if h.lower() == "set-cookie"]
...
async def test_cross_tenant_isolation_different_callers_dont_collide(call_counter) -> None:
    ...
    assert "idempotency-replayed" not in {k.lower() for k in r_a.headers}
    assert "idempotency-replayed" not in {k.lower() for k in r_b.headers}
