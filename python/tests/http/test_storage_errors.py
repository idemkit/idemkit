...
- ``fail_closed`` (default): reject with HTTP 503 + Retry-After + Problem Details,
  and emit ``idempotency.storage_error`` (the request was refused).
- ``fail_open``: pass through (request runs without idempotency protection), and
  emit ``idempotency.ran_unprotected`` so the unprotected-run count is measurable.
"""
...
from tests.fakes import FailingBackend
...
async def test_fail_open_passes_through() -> None:
    """fail_open: handler runs without idempotency (no effective_key returned), and
    emits ran_unprotected (distinct from storage_error) so the unprotected-run count
    is measurable separately from refusals."""
    ...
    assert any(e.decision == Decision.RAN_UNPROTECTED for e in events)
    assert not any(e.decision == Decision.STORAGE_ERROR for e in events)
