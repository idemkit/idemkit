...
def _build_app(handler, *, redactor) -> Starlette:
    ...
    app.add_middleware(
        IdempotencyMiddleware, backend=InMemoryBackend(), config=config
    )
    return app
...
async def test_redactor_modifies_stored_response_not_first_response() -> None:
    ...
    async def handler(request: Request) -> Response:
        return JSONResponse(
            {"card": "4111-1111-1111-1111", "amount": 100}, status_code=201
        )
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        r1 = await client.post(
            "/echo", json={}, headers={"Idempotency-Key": "k-pci"}
        )
...
async def test_redactor_can_strip_headers() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        r1 = await client.post(
            "/echo", json={}, headers={"Idempotency-Key": "k-strip"}
        )
...
async def test_redactor_exception_does_not_crash_request_and_does_not_store() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        r1 = await client.post(
            "/echo", json={}, headers={"Idempotency-Key": "k-broken-r"}
        )
        r2 = await client.post(
            "/echo", json={}, headers={"Idempotency-Key": "k-broken-r"}
        )
    ...
    assert "idempotency-replayed" not in {k.lower() for k in r2.headers.keys()}
