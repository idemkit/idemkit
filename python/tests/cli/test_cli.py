...
pytestmark = pytest.mark.skipif(not PG_URL, reason="set IDEMKIT_TEST_PG_URL to enable CLI tests")
...
async def _table_exists() -> bool:
    conn = await asyncpg.connect(PG_URL)
    try:
        result = await conn.fetchval("SELECT to_regclass('public.idemkit_records')")
